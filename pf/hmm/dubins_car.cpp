...
#define GAMMA               ((max_states[3] - min_states[3]))
...
// limits of states
double max_states[4] = {0.1, 0.5, 0.5, PI/4};
double min_states[4] = {0.0, 0.0, 0.0, -PI/4};
...
double get_edge_prob_obs(state xinit, double till_obs, state obs)
{
    ...
    if (NUM_DIM == 2)
        volume_parts = 3*var[0];
    else if( NUM_DIM == 3)
        volume_parts = PI*9*(var[0])*(var[1]);
    else if( NUM_DIM == 4)
        volume_parts = 4*PI/3*27*(var[0])*(var[1])*(var[2]);
    
    retval = volume_parts*retval/(NUM_PARTICLES - 1);
