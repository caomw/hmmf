...
System::System()
{
    ...
    max_states[0] = 1; max_states[1] = 1; max_states[2] = 1; max_states[3] = M_PI_2;
...
State System::integrate(State& s, double duration, bool is_clean)
{
    ...
    {
        t.x[0] += sim_time_delta;
        t.x[1] += 1.0*cos(t.x[3])*sim_time_delta;
        t.x[2] += 1.0*sin(t.x[3])*sim_time_delta;
        t.x[3] += 1.0*sim_time_delta;
