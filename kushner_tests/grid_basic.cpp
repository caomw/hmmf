...
int main(int argc, char** argv)
{
    ...
    for(int num = 100; num<50000; num=num+1000)
    {
        num_states = num;
        ...
        for(int i=0; i<num_states; i++)
        {
            if ( (i!= 0) &&(i!=(num_states-1)))
            {
                ...
                */
                ...
                /*
                // fixed formulae
                edge_probs[i][0] = (pvar/2.0 + h*fabs(drift(states[i])))/c;
                edge_probs[i][1] = pvar/2.0/c;
                */
            }
            else if(i == 0)
            {
        ...
        for(int i=0; i< num_traj; i++)
        {
            ...
            {
                double cointoss = RANDF;
                ctime += htimes[si];
                if(cointoss < edge_probs[si][0])
                {
